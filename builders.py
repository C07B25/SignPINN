...
def build_scheduler(config: dict, optimizer: Optimizer, scheduler_mode: str,
    ...
    :param optimizer: optimizer for the scheduler
    :param scheduler_mode: "min" or "max" for plateau
    :param hidden_size: encoder hidden size (required for NoamScheduler)
    :return:
        - scheduler object,
        - scheduler_step_at: "validation", "epoch", or "step"
    """
    ...
    if "scheduling" in config.keys() and config["scheduling"]:
        if config["scheduling"].lower() == "plateau":
            ...
            if hidden_size <= 0:
                raise ConfigurationError("NoamScheduler requires hidden_size > 0")
